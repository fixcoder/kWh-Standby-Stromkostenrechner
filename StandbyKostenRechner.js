...
function EuroProJahr() {
    ...
    let LableText1 = "Betrieb oder Standbybetrieb in <u>Stunden</u><span style='background-color:yellow;'> an jedem Tag im Jahr</span>";
    let LableText2 = "Stromkosten &euro; <span style='background-color:yellow;'>im Jahr</span> (Grundpreis ca. 13,90 &euro;/Monat nicht mit einberechnet.)";

    let iWatt = HoleNummerVonInputFeldVMit("idWatt");

    iWatt = eval(iWatt);
    document.getElementById('idWatt').value = iWatt;

    let iCent = HoleNummerVonInputFeldVMit("idCent");
    let iStunden = HoleNummerVonInputFeldVMit("idStunden");
    ...
    let strHelp = str_ikWh + " kWh. (+ " + str_eurAb + " EUR monatl. Abschlag.)";

    // volle 24 standbybetriebsstunden täglich im Jahr
    if (((iAKW > 0.4) && (iStunden === 24)) || ((iAKW > 0.4) && (iStunden === 8760))) {
        document.getElementById("BRD").innerHTML = "= " + strHelp +
			"<span style='color:red;'>Bei 40,68 Mio. Haushalten (kWh pro Jahr) in der BRD entspricht das " +
			iAKW.replace(".", ",") + " AKWs (mittl. AKW mit 1400 MegaWatt).</span>"
    } else {
        document.getElementById("BRD").innerHTML = strHelp;
    }
...
function calculate(id, idresult) {
    ...
    try {
        document.getElementById(idresult).value = eval(formulaInput);
    } catch (error) {
...
function kWhProJahr() {
    ...
    if (ikWh !== '') {
        ...
        if (ih === '') {
            ih = 24;
            document.getElementById('idStunden').value = 24;
        }
        ih = ih.replace(",", ".");
        ih = parseFloat(ih);
        ikWh = ikWh.replace(",", ".");
...
function StandByKostenRechnerW_C_h(iWatt, iCent, ih) {
    ...
    if (iCent !== '') {
        document.getElementById('idCent').value = iCent;
    }
    if (ih !== '') {
        document.getElementById('idStunden').value = ih;
    }
